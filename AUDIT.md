...
- [ ] Wheel of Fortune spin result isn't announced to screen readers — `showResult()` just unhides `#result-card` with no `aria-live`/`role="status"`. Add `aria-live="polite"` to `#result-card`. (found: 2026-09-01)
...
- [ ] `index.astro:329-331` has commented-out markup (a `<p class="small-muted">` referencing "Croydon Tourist Board") left in the production file. Remove it or restore it. (found: 2026-09-01)
